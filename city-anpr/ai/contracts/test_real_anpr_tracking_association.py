...
def _candidate_count(
    plate: PlateObservation, vehicles: Sequence[TrackedVehicle]
) -> int:
    """Count containment candidates using the association layer's geometry rules."""
...
def _coverage_buckets(values: List[float]) -> Dict[str, int]:
    return {
        "0.90-0.95": sum(0.90 <= value < 0.95 for value in values),
        "0.95-0.99": sum(0.95 <= value < 0.99 for value in values),
        "0.99-1.00": sum(0.99 <= value <= 1.00 for value in values),
    }
...
def _update_history(
    ...
    if track_result.boxes is None or track_result.boxes.id is None:
    ...
    for index in range(len(boxes)):
        ...
        if track_id not in history:
            ...
            record["best_confidence"] = max(record["best_confidence"], confidence)
            record["bbox"] = [x1, y1, x2, y2]
            record["center"] = center
            record["trajectory"].append(center)

        active_records.append(dict(history[track_id]))
...
def run_measurement() -> Dict[str, Any]:
    ...
    if not VIDEO_PATH.is_file() or not MODEL_PATH.is_file():
        raise FileNotFoundError("Required traffic video or YOLO model is unavailable.")

    raw_frames = cv2.VideoCapture(str(VIDEO_PATH))
    if not raw_frames.isOpened():
        raise RuntimeError(f"Could not open video: {VIDEO_PATH}")

    tracker = VehicleTracker(model_path=str(MODEL_PATH))
    anpr = ANPRPipeline()
    track_results = tracker.track_video(str(VIDEO_PATH))
    history: Dict[int, Dict[str, Any]] = {}
    ...
    try:
        for frame_number, track_result in enumerate(track_results, start=1):
            ok, frame = raw_frames.read()
            if not ok:
                raise RuntimeError(
                    "Raw frame reader ended before the tracking result stream."
                )
            frames_processed = frame_number

            active_records = _update_history(
                history, tracker, track_result, frame_number
            )
            tracked_vehicles = [
                tracked_vehicle_from_dict(record, camera_id=CAMERA_ID)
                for record in active_records
            ]

            anpr_results = anpr.detect_and_read(frame)
            plate_observations = [
                PlateObservation(
            ...
            total_plates += len(plate_observations)
            ocr_confidences.extend(plate.ocr_confidence for plate in plate_observations)
            detection_confidences.extend(
                plate.detection_confidence for plate in plate_observations
            )

            for plate in plate_observations:
                candidates = _candidate_count(plate, tracked_vehicles)
                if candidates == 0:
                    no_containing_vehicle += 1
                    print(f"Frame {frame_number}: plate has no containing vehicle")
                elif candidates > 1:
                    multiple_candidates += 1
                    print(
                        f"Frame {frame_number}: plate has {candidates} containing vehicles"
                    )
                if not plate.plate_text:
                    empty_ocr_text += 1
                    print(f"Frame {frame_number}: plate OCR text is empty")
                if plate.ocr_confidence < LOW_OCR_CONFIDENCE:
                    low_ocr_confidence += 1
                    print(
                        f"Frame {frame_number}: low OCR confidence "
                        f"({plate.ocr_confidence:.3f})"
                    )

            associated_observations = associate_plates_to_vehicles(
                tracked_vehicles,
                plate_observations,
                minimum_coverage=MINIMUM_COVERAGE,
                vehicle_bbox_expansion=VEHICLE_BBOX_EXPANSION,
            )
            vehicles_by_id = {vehicle.track_id: vehicle for vehicle in tracked_vehicles}
            ...
            for observation in associated_observations:
                ...
                associated_plates += 1
                coverage_scores.append(observation.association_confidence or 0.0)
                vehicle = vehicles_by_id[observation.track_id]
                associations_by_vehicle_type[vehicle.vehicle_type] += 1
                print(
                    "GEOMETRIC ASSOCIATION "
                    f"frame={frame_number} track_id={observation.track_id} "
                    ...
                    f"coverage={observation.association_confidence:.3f} "
                    f"ocr_confidence={observation.ocr_confidence:.3f}"
                )
    finally:
        raw_frames.release()
    ...
    association_rate = associated_plates / total_plates if total_plates else 0.0
    result = {
        ...
        "unassociated_plate_observations": unassociated_plates,
        "association_rate": association_rate,
        "coverage_scores": _summary(coverage_scores),
        "coverage_buckets": _coverage_buckets(coverage_scores),
        "associations_by_vehicle_type": dict(associations_by_vehicle_type),
        "ocr_confidence": _summary(ocr_confidences),
        "plate_detection_confidence": _summary(detection_confidences),
        "no_containing_vehicle": no_containing_vehicle,
    ...
    print("ANPR-to-tracking geometric association summary:")
